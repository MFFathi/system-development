...
class LoginGUI():
        ...
        def getLogin(self,username_entry, password_entry, widgets,branch,root):
                ...
                if verifyLogin.verify(entered_username, entered_password,widgets,branch,root) == TRUE:    
                    for x in widgets:
                        x.pack_forget()
                        
                elif (verifyLogin.verify(entered_username, entered_password,widgets,branch,root) == FALSE): #If The Entered Details Are Not Correct, Then A New Window Will Pop-up And Display It To The User
